/**
 * Downloads the sidecar binaries into resources/bin/<os>/ — ffmpeg (import
 * normalization), on Windows/Linux whisper-cli + parakeet-cli (transcription,
 * see src/main/services/whisper.ts and parakeet.ts), and
 * sherpa-onnx-offline-speaker-diarization on all three platforms (speaker
 * detection, see src/main/services/diarize.ts) — plus its two small ONNX
 * models into resources/models/ (platform-independent, fetched once).
 *
 * Deliberately not committed: they're large, platform-specific and
 * separately licensed. Run this after cloning, and once per target platform
 * you intend to package for.
 *
 ...
 *
 * Licensing note: the Windows and Linux ffmpeg builds are LGPL. No LGPL
 * macOS build is published by BtbN, so macOS uses the GPL build from
 * ffmpeg-static. ffmpeg is invoked as a separate process and is never linked
 * into the app, but if you ship the macOS build commercially, review that
 * obligation.
 */
...
// Pinned so a rebuild months from now produces the same binaries.
const FFMPEG_BTBN_TAG = 'latest'
const FFMPEG_BTBN_SERIES = 'n8.1'
const FFMPEG_STATIC_TAG = 'b6.1.1'

// whisper.cpp publishes prebuilt CLIs for Windows and Linux only, and its
// release bundles whisper-cli and parakeet-cli together.
 ...
 * @property {string} name          Label used in logs, and the filename for a single-member artifact.
 * @property {string} url
...
/**
 * Diarization's two ONNX models — small, platform-independent data rather
 * than a binary, bundled with the app instead of downloaded at runtime (see
 * services/sidecars.ts's resolveBundledModel). Fetched once regardless of
 * which `--os` is being packaged.
 *
 * @returns {Artifact[]}
 */
...
function artifactsFor(os, arch) {
  ...
  const sherpa = (asset) => `https://github.com/k2-fsa/sherpa-onnx/releases/download/${SHERPA_TAG}/${asset}`
  ...
  if (os === 'win') {
    ...
    return [
      ...
      {
        // MT links the CRT statically, so the app does not require the user
        // to install a Visual C++ redistributable.
  ...
  if (os === 'mac') {
    ...
    // ffmpeg and diarization are fetched; whisper-cli/parakeet-cli have to
    // come from Homebrew or a source build, handled by noteMacWhisper()
    // below.
    return [
...
/**
 * Extracts an artifact into `targetDir`. A `members` artifact places every
 * matching file directly in `targetDir` (flattening whatever subdirectory
 * the archive used); a `member`/raw/gz artifact writes to `destPath`.
 */
async function extract(artifact, archivePath, workDir, destPath, targetDir) {
  ...
  // bsdtar ships with Windows 10+, macOS and most Linux distros, and reads
  // both zip and tar.xz — which avoids pulling in an archive library.
  //
  // On Windows the system bsdtar must be addressed by absolute path: if a
  // POSIX shell like Git Bash is on PATH its GNU tar wins, and GNU tar reads
  // the "C:" in a Windows path as a remote host and tries to connect to it.
  const tarBin =
  ...
  if (artifact.members) {
    ...
    if (found.length === 0) {
      throw new Error(`None of [${artifact.members.join(', ')}] found inside ${artifact.url}`)
    }
    for (const file of found) {
      await moveFile(file, join(targetDir, basename(file)))
    }
    return
  }
 ...
 *
 * `rename` is atomic but POSIX (and Windows) refuse it across devices —
 * EXDEV — and the OS temp directory is not guaranteed to share a drive with
 * the repo. It usually does on a dev machine, which is why this only
 * surfaces on CI runners that put TEMP and the checkout on separate drives.
 */
 ...
 * others are. Both routes below put whisper-cli/parakeet-cli somewhere the
 * app will find them — the sidecar resolver falls back to PATH.
 */
function noteMacWhisper() {
  console.log('')
  console.log('  whisper-cli / parakeet-cli: no prebuilt macOS binary is published upstream.')
  console.log('  Install whisper-cli one of these ways:')
  console.log('')
  console.log('    brew install whisper-cpp')
  console.log('')
  console.log('  or build both from source (gives Metal acceleration, and is the only')
  console.log('  known route to parakeet-cli on macOS):')
  console.log('')
  ...
  console.log('    cp build/bin/whisper-cli build/bin/parakeet-cli resources/bin/mac/')
  console.log('')
}
...
async function main() {
  ...
  const artifacts = artifactsFor(os, arch)
  console.log(`Fetching sidecars for ${os}-${arch} -> resources/bin/${os}/ (and resources/models/)`)

  for (const artifact of artifacts) {
    const targetDir = artifact.dest === 'models' ? modelsDir : binDir
    const destPath = join(targetDir, artifact.name)
    const sentinelPath = join(targetDir, artifact.sentinel ?? artifact.name)

    if (!force && (await exists(sentinelPath))) {
      console.log(`  ${artifact.name} already present (use --force to re-download)`)
      continue
    }

    const workDir = await mkdtemp(join(tmpdir(), 'sonascribe-sidecar-'))
    try {
      console.log(`  downloading ${artifact.name}…`)
      const archivePath = join(workDir, 'download.bin')
      await download(artifact.url, archivePath)
      await extract(artifact, archivePath, workDir, destPath, targetDir)
      // The zip/tar members and the raw macOS build are not reliably +x.
      // Models need no executable bit at all.
      if (os !== 'win' && artifact.dest !== 'models') {
        const madeExecutable = artifact.members
          ? await findMatching(targetDir, artifact.members.filter((p) => !p.startsWith('*.')))
          : [destPath]
        for (const file of madeExecutable) await chmod(file, 0o755).catch(() => undefined)
      }
      const { size } = await stat(sentinelPath)
