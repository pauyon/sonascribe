 ...
 * what "small.en" means. Deliberately short — a handful of curated options
 * spanning low-end to high-end hardware, not the full upstream model zoo, so
 * picking one doesn't require already knowing what a quantized GGML file is.
 */
...
export interface EngineSpec {
  ...
  label: string
  blurb: string
  /** Which sidecar binary this engine needs — see `services/sidecars.ts`. */
  sidecar: 'whisper-cli' | 'parakeet-cli'
}
...
export const DEFAULT_ENGINE: AsrEngine = 'parakeet'
export const DEFAULT_MODEL_ID = 'parakeet-tdt-0.6b-v3-q8_0'

/** Which model to suggest for an engine the user hasn't picked one for yet. */
export function defaultModelForEngine(engine: AsrEngine): string {
  return engine === 'parakeet' ? DEFAULT_MODEL_ID : 'small.en'
}
...
export function modelsForEngine(engine: AsrEngine): ModelSpec[] {
  return MODELS.filter((m) => m.engine === engine)
}

export function findEngine(id: AsrEngine): EngineSpec {
...
export interface ModelStatus {
  ...
  bytesOnDisk: number
  downloading: boolean
  /** 0..1 while downloading, or null when the total size isn't known yet. */
