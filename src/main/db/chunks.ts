 ...
 * than patched incrementally — `saveTranscript`/`saveSpeakerMergedTranscript`
 * already delete and re-insert every utterance with fresh ids on each run, so
...
/** Chunk count and how many distinct recordings have at least one — drives the Settings "N chunks indexed" line. */
export function getChunkIndexStats(): { chunkCount: number; indexedRecordings: number } {
  const db = getDb()
  const row = db
    .prepare(
      'SELECT COUNT(*) AS chunk_count, COUNT(DISTINCT recording_id) AS indexed_recordings FROM chunk_embeddings'
    )
    .get() as unknown as { chunk_count: number; indexed_recordings: number }
  return { chunkCount: row.chunk_count, indexedRecordings: row.indexed_recordings }
}
