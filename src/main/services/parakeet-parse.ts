 ...
 * upstream change to the table format would silently produce empty
 * transcripts — and a parser that can only be tested by running a 356 MB
 * model would not be tested at all.
 */
 ...
 *
 * `word_start` is authoritative, so unlike the whisper path there is no need
 * to infer boundaries from leading whitespace. Punctuation arrives as its
 * own token with word_start=false and a zero-length span, and is appended to
 * the word before it.
 */
export function parseTokenTable(output: string): TranscriptWord[] {
  ...
  for (const line of output.split('\n')) {
    ...
    // Vocabulary control tokens (<blk>, <unk>) are angle-bracketed and carry
    // no transcript text. Current builds do not print them, but a blank
    // token reaching the fallback below would otherwise be emitted as a
    // real word.
