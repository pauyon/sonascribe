 ...
 *
 * Utterances are speaker turns, not fixed-size units — a monologue with no
 * pause is one long paragraph, a rapid exchange is several one-word rows.
 * Neither is a good embedding chunk on its own: short turns are merged
 * together up to a target size, long ones are split at sentence boundaries.
 */
