 ...
 *
 * The CLI writes JSON to a file rather than stdout, so a temporary directory
 * is used per run and cleaned up afterwards. Progress is parsed from stderr.
 * Unlike Parakeet, whisper.cpp windows long audio internally and reports
 * real incremental progress, so it needs none of the chunking in
 * `audio-chunks.ts` — it is simply handed the whole file.
 */
...
/**
 * Whisper emits sub-word tokens (" fell", "ow"), plus bracketed control
 * tokens such as `[_BEG_]`. A new word starts at a token beginning with
 * whitespace; everything else attaches to the token before it, which is what
 * keeps contractions and trailing punctuation intact.
 */
function tokensToWords(
  ...
  for (const token of tokens) {
    ...
    // [_BEG_], [_EOT_], [_SOT_] and the like, but also timestamp tokens such
    // as [_TT_480], which have no trailing underscore. Matching only the
    // first shape leaks timestamps into the transcript wherever text is
    // rebuilt from words rather than taken from whisper's own segment text.
    ...
    // Keep the weakest token's probability — a word is only as trustworthy
...
function parseWhisperJson(json: WhisperJson): TranscriptionResult {
  ...
  for (const raw of json.transcription ?? []) {
    ...
    if (!text) continue
    // Whisper annotates non-speech as a bracketed placeholder —
    // [BLANK_AUDIO], [ Silence ], (upbeat music). These are not things
    // anyone said, and they read as a real utterance once rendered.
...
export async function transcribeWithWhisper(
  ...
  const workDir = await mkdtemp(join(tmpdir(), 'sonascribe-whisper-'))
