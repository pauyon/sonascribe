 ...
 *
 * Two models are involved: pyannote-segmentation-3.0 decides *when* somebody
 * is speaking, and a speaker-embedding network decides *who*. Embeddings are
 * then clustered — either into a known number of speakers, or by a distance
...
export interface DiarizeOptions {
  wavPath: string
  /**
   * Speaker count when known. More accurate than thresholding — most
   * meetings know their own headcount.
   *
   * A ceiling rather than an exact figure, despite the flag's name: asking
   * for 3 clusters on audio holding two voices returns 2, and asking for 5
   * returns 4. It stops the count running away without inventing speakers
   * to fill a quota.
   */
  numSpeakers?: number | null
  /** Distance threshold used when the count is unknown. Lower splits more eagerly (more speakers), higher merges more. */
  threshold?: number
  /**
   * Segments shorter than this many seconds are discarded before clustering.
   *
   * A 0.3s blip — a cough, a keystroke, half a "mm-hm" — is long enough to
   * become a segment but far too short for a reliable speaker embedding, so
   * it clusters as its own person. Discarding those costs a little real
   * speech at the margins and removes most invented speakers.
   */
  minDurationOn?: number
  /** Length of the audio, used to temper minDurationOn. Optional: without it the preset value is used unchanged. */
  durationMs?: number
  /**
   * Threads for both neural networks.
   *
   * sherpa-onnx defaults each to 1; passing a real number uses more of the
   * machine with byte-identical output.
   */
  threads?: number
  /**
   ...
   * length — 90% overlap, and roughly ten times the segmentation work
   * needed. 0.25 is 2.2x faster and returns the same speaker counts on
   * sherpa's own reference recordings; 0.5 is faster still and misses a
   * speaker.
   */
 ...
 *
 * Measured against sherpa-onnx's own reference recordings — a 2-speaker
 * clip and a 4-speaker clip — 'balanced' is the only setting that returns
 * the right count for both. The library's own defaults (0.5/0.3) report 8
 * speakers for the 4-speaker clip; 'merge' fuses that clip down to 3.
 */
export const SPLITTING_PRESETS: Record<SpeakerSplitting, { threshold: number; minDurationOn: number }> = {
 ...
 *
 * Discarding sub-second segments is what stops a long meeting fragmenting
 * into a tail of invented speakers. On a short take it does the opposite: a
 * 7-second recording where the second person says one word for 0.39s loses
 * that person entirely. Below a minute the floor drops to 0.25s, easing
 * back to the preset value by the two-minute mark.
 */
