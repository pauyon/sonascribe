 ...
 * printed no tokens, which reads downstream as "this audio has no speech in
 * it". A recording full of conversation would otherwise be reported silent.
 *
 * Memory tracks the length of the file handed over, at roughly 800 MB per
 * minute of audio once the model itself is loaded. Ten-minute windows
 * measured 10.1 GB resident, where the whole file wanted 66 GB and got
 * nothing done. whisper.cpp windows audio internally and needs none of this;
 * only the Parakeet path uses this module.
 */
 ...
 *
 * Splitting is not free — it re-encodes, and every boundary is a chance to
 * lose a word across the seam — so it is only worth doing once a file is
 ...
 * instead.
 *
 * -50 dB over 0.4 s is a pause between sentences, not the gap between two
 * words — the aim is a handful of good boundaries, not every hesitation.
 */
 ...
 *
 * Each boundary starts at a multiple of the target length and then moves to
 * the middle of the nearest silence within the search window. Falling back
 * to the exact offset is fine: a boundary in the middle of speech costs at
 * most the word it lands on, where not splitting at all costs the entire
 * recording.
 */
export function planBoundaries(
  ...
  for (let at = targetMs; at < durationMs; at += targetMs) {
    ...
    for (const silence of silences) {
      ...
      // Never move a boundary backwards past one already chosen, or the
      // windows would overlap and duplicate speech.
 ...
 * fragments in the user's media folder that later look like real files.
 */
  ...
  return Promise.all(
    edges.slice(0, -1).map((startMs, i) => {
      ...
      return runFfmpegCapture(
        [
          ...baseArgs(),
          // Before -i so ffmpeg seeks rather than decoding and discarding; on
          // PCM this is exact, so the offsets stay sample-accurate.
          '-ss',
 ...
 *
 * The split files are deleted in a `finally` when a job ends, but a job
 * killed with the app never reaches it: quitting mid-transcription strands a
 * few hundred megabytes of scratch WAVs in the temp directory. Jobs do not
 * survive a restart, so anything found here at startup belongs to a run
 * that is already over and is safe to remove.
 */
