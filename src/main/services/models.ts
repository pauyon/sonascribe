...
import { MODELS, findModel, modelDownloadUrl, type ModelSpec, type ModelStatus } from '@shared/models'
import { modelsPath } from '../paths'
import { emit } from '../ipc/events'

/**
 * Download and inventory management for ASR models.
 *
 * Models are large (78 MB to 1.6 GB) and are therefore not bundled — the
 * installer stays small and the user picks what suits their machine.
 * Downloads are resumable, because a transfer that size having to restart
 * from zero after a dropped connection is not something a user will
 * tolerate twice.
 */
...
function finalPath(id: string): string {
  return join(modelsPath(), specFor(id).file)
}
 ...
 *
 * On disk the bytes therefore read "lmgg", not "ggml" — comparing the
 * leading four bytes as ASCII rejects every valid model.
 */
 ...
 * later.
 */
async function hasValidMagic(path: string): Promise<boolean> {
  try {
...
export async function isModelInstalled(id: string): Promise<boolean> {
  ...
  return (await sizeOf(path)) > 0 && (await hasValidMagic(path))
}
  ...
  return Promise.all(
    MODELS.map(async (spec) => {
      ...
      return {
        ...
        fraction: download?.totalBytes ? download.receivedBytes / download.totalBytes : null,
        error: null
      }
    })
  )
 ...
 *
 * Returns the final path. Concurrent calls for the same model share the
 * first download rather than racing each other onto the same file.
 */
export async function downloadModel(id: string): Promise<string> {
  ...
  try {
    ...
    if (!response.ok) {
      throw new ModelDownloadError(`Download failed: ${response.status} ${response.statusText}`)
    }
    if (!response.body) throw new ModelDownloadError('Download returned an empty body')

    // 206 means the server honoured the resume; 200 means it ignored the
    // Range header and is sending the whole file, so any partial data must
    // be dropped.
    ...
    // A cancelled or network-interrupted download keeps its .part file so
    // the next attempt can resume; only a corrupt payload is discarded
    // (above).
    emit('model:progress', {
      modelId: id,
      receivedBytes: state.receivedBytes,
 ...
 * an open file rather than deferring it the way Unix does. This is a
 * user-requested delete, and silently leaving the model on disk while
 * claiming success would be worse than a delete the user can just retry.
 */
