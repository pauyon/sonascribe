 ...
 * downstream (segment grouping, persistence) works on this shape rather than
 * on either engine's native output. Adding a third engine means writing one
 * runner, not touching the pipeline.
 */
...
export interface TranscribeOptions {
   ...
   * CLI prints no progress, so its jobs are indeterminate per-window; the
   * caller that splits a long file into windows derives an overall fraction
   * from how many windows have finished instead.
   */
 ...
 *
 * Parakeet emits one flat token stream with no segmentation of its own.
 * Rather than hand the rest of the app a single enormous segment, split on
 * silence — this is what turns into paragraphs for display
 * (`TranscriptPanel.tsx`'s `paragraphize`) and into separately-timestamped
 * rows in the `utterances` table.
 */
