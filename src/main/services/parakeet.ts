...
import { runFfmpegCapture, baseArgs, ASR_CHANNELS, ASR_SAMPLE_RATE } from './ffmpeg'
 ...
 *
 * Measured on 20 minutes of audio in four 5-minute windows, 8-core Core Ultra
 * 7, CPU backend: 273 s serially at 6 threads, 234 s with four workers at 2
 * threads, 221 s with two workers at 4 threads. Token counts were identical
 * in all three, so the parallelism costs nothing in accuracy — but two
 * workers is only 19% faster than serial and four is slower than two.
 *
 * That shape says the work is bound by memory bandwidth rather than by
 * cores: more workers mostly buy more contention for the same bus. Worth
 * taking, and worth being honest that a large speedup lives in a GPU backend
 * instead.
 *
...
function planWorkers(chunkMs: number, coreBudget: number): { workers: number; threads: number } {
  ...
  // Only part of what is free, so a transcription cannot squeeze everything
  // else on the machine into swap the way the unsplit file did.
 ...
 *
 * The engine holds an entire file in memory at once and its appetite grows
 * with the length of that file — see `audio-chunks.ts` for the measured
 * numbers. Splitting also buys real progress: a single call reports none at
 * all, so the UI could only show indeterminate work for however long a long
 * file took; finished windows are a genuine fraction.
 */
export async function transcribeWithParakeet(
  ...
  try {
    ...
    const worker = async (): Promise<void> => {
      for (;;) {
        ...
        try {
          ...
          // actually said. System audio is silent for most of a call, so
          // this check (already used on the tail sweep below) applies just
          // as well to a whole 5-minute window that turned out to hold no
          // signal.
          const { peak } = await measureLevels(chunks[index].path)
          ...
          results[index] =
            peak < SILENCE_PEAK_THRESHOLD
              ...
              : await transcribeOneFile({
                  ...
                  threads,
                  // A single run reports nothing usable, so progress is
                  // counted in completed windows instead.
                  onProgress: () => undefined
                })
        } catch (err) {
          // Remember the first failure and stop drawing work; the windows
          // still running will finish on their own and be discarded.
          failure ??= err
          return
        }
    ...
    // Each window was transcribed as if it began at zero, so its timings
    // have to be moved back onto the recording's timeline before they are
    // merged.
 ...
 *
 * A 1.97-second slice carrying five clearly spoken words came back empty;
 * the same slice padded to 2.5 s returned all five, and padded to 4 or 5 s
 * returned them again. Padded to 3 s it returned two, so this is not a clean
 * threshold — the decoder is simply unreliable on very short input, and
 * every padded length tried beat the unpadded original. Silence at the end
 * costs a fraction of a second of compute and cannot add words that were not
 * spoken.
 */
 ...
 *
 * The slice is padded up to MIN_ENGINE_INPUT_MS, and anything the engine
 * claims to hear inside that padding is dropped — silence cannot contain
 * words, so a word timed past the real end of the slice is the decoder
 * inventing one.
 */
async function transcribeTail(
  ...
  try {
    ...
    if ((await measureLevels(tailPath)).peak < SILENCE_PEAK_THRESHOLD) return null
 ...
 * when handed that slice on its own. On a 4.61-second recording it stopped
 * at 2.64 s and left five spoken words behind.
 *
 * So the end is checked rather than trusted: while the transcript stops
 * well short of the audio and what remains is not silence, that remainder
 * is transcribed on its own and appended. It loops because a sweep can stop
 * early for the same reason the first pass did.
 */
